# MizanCore\BaasSdk\Generated\BaaSBalanceApi

All URIs are relative to https://api.mizancore.ng, except if the operation defines another base path.

| Method | HTTP request | Description |
| ------------- | ------------- | ------------- |
| [**baasBalanceBalance()**](BaaSBalanceApi.md#baasBalanceBalance) | **GET** /api/v1/baas/balance | Get settlement account balance |
| [**baasBalanceFeePreview()**](BaaSBalanceApi.md#baasBalanceFeePreview) | **POST** /api/v1/baas/withdrawals/fee-preview | Preview withdrawal fee |
| [**developerWithdrawalsFeePreview()**](BaaSBalanceApi.md#developerWithdrawalsFeePreview) | **POST** /api/v1/developer/withdrawals/fee-preview | Preview withdrawal fee |
...
$x_tenant_id = world.test.localhost; // string | Tenant identifier (UUID or domain, e.g. world.test.localhost). Required on every tenant-scoped route. Maps to the tenant whose database serves this request. In production, prefer Host-header-based resolution; X-Tenant-ID is intended for non-production environments and is rejected (HTTP 400) on production hosts.
$fee_preview_request = {"amount":100000}; // \MizanCore\BaasSdk\Generated\Model\FeePreviewRequest

try {
    $result = $apiInstance->baasBalanceFeePreview($x_tenant_id, $fee_preview_request);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling BaaSBalanceApi->baasBalanceFeePreview: ', $e->getMessage(), PHP_EOL;
}
```
...
| ------------- | ------------- | ------------- | ------------- |
| **x_tenant_id** | **string**| Tenant identifier (UUID or domain, e.g. world.test.localhost). Required on every tenant-scoped route. Maps to the tenant whose database serves this request. In production, prefer Host-header-based resolution; X-Tenant-ID is intended for non-production environments and is rejected (HTTP 400) on production hosts. | |
| **fee_preview_request** | [**\MizanCore\BaasSdk\Generated\Model\FeePreviewRequest**](../Model/FeePreviewRequest.md)|  | |
...
$x_tenant_id = world.test.localhost; // string | Tenant identifier (UUID or domain, e.g. world.test.localhost). Required on every tenant-scoped route. Maps to the tenant whose database serves this request. In production, prefer Host-header-based resolution; X-Tenant-ID is intended for non-production environments and is rejected (HTTP 400) on production hosts.
$fee_preview_request = {"amount":150050}; // \MizanCore\BaasSdk\Generated\Model\FeePreviewRequest

try {
    $result = $apiInstance->developerWithdrawalsFeePreview($x_tenant_id, $fee_preview_request);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling BaaSBalanceApi->developerWithdrawalsFeePreview: ', $e->getMessage(), PHP_EOL;
}
```
